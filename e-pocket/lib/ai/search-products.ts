...
import { getStockStatus, getStockStatusLabel } from '@/lib/constants/stock';
import type { AI_SEARCH_PRODUCTS_QUERYResult } from '@/sanity.types';
import type { SearchProduct } from '@/lib/ai/types';
import { AI_SEARCH_PRODUCTS_QUERY } from '@/sanity/queries/product.query';
import { COLOR_VALUES, MATERIAL_VALUES } from '../constants/filter';
  ...
  execute: async ({ query, category, material, color, minPrice, maxPrice }) => {
    ...
    try {
      ...
      const formattedProducts: SearchProduct[] = (
        ...
        stockStatus: getStockStatus(product.stock)?.replace(
          /-/g,
          '_',
        ) as SearchProduct['stockStatus'],
        stockMessage: getStockStatusLabel(product.stock)?.replace(/-/g, '_'),
